...
def save_tc_results(tc_stats, tcs):
    ...
    with open(os.path.join(cf.files["stats_path"], dt_string + "-stats.json"), "w") as f:
        json.dump(tc_stats, f, indent=4)
        print("Stats saved as %s" % os.path.join(cf.files["stats_path"], dt_string + "-stats.json"))


    with open(os.path.join(cf.files["tcs_path"], dt_string + "-tcs.json"), "w") as f:
        json.dump(tcs, f, indent=4)
        print("Test cases saved as %s" % os.path.join(cf.files["tcs_path"], dt_string + "-tcs.json"))